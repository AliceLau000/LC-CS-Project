# Hello!!! Welcome to my model with an adaptive system
...
watering_rate = 0 # adaptive watering intensity
...
def update_forest(forest, temp, light, size, fire, burnt_tree, normal_tree, normal_leave, dead_tree, dead_leave,
  ...
  # ADAPTIVE SYSTEM: water sprinkler system
  # determine the water sprinkling zone around fires
  dynamic_zone = []

  if current_fires: # if there is active fire cells, find the centre of all fire
    avg_r = sum(f[0] for f in current_fires) // len(current_fires) # get the average row of all fire cells
    avg_c = sum(f[1] for f in current_fires) // len(current_fires) # get the average column of all fire cells

    # define sprinkler radius based on amount of fire that exists
    radius = 4
    if len(current_fires) > 50:
      radius = 7
    elif len(current_fires) > 225:
      radius = 9

    # build adaptive water zone grid
    for r in range(avg_r - radius, avg_r + radius + 1):
      for c in range(avg_c - radius, avg_c + radius + 1):
        if 0 <= r < size and 0 <= c < size:
          dynamic_zone.append((r, c))

  # set watering_rate based on risk level
    ...
    for j in range(size):
      ...
      if cell in [normal_tree, normal_leave, dead_tree, dead_leave]:
        ...
        if neighbours > 0:
          ...
          # apply watering reduction inside sprinkler zone
          if watering_rate > 0 and (i,j) in dynamic_zone:
            prob *= (1 - watering_rate)
  ...
  return new_forest, dynamic_zone
...
def forest_simulation(temp,light):
  ...
  while fire_exists(forest, fire):
    ...
    # Update forest state
    forest, sprinkler_zone = update_forest(forest, temp, light, size,fire, burnt, normal_tree, normal_leave, dead_tree, dead_leave, burn_time, 
    ...
    for i in range(size):
      ...
      for j in range(size):
        ...
        # apply sprinkler zone visual effect
        if watering_rate > 0 and (i,j) in sprinkler_zone:
          if cell != "F":
            r = int(r * 0.6) 
            g = int(g * 0.6)
            b = min(255, b + 100)
    ...
    if "Negligible" not in current_risk and "Minor" not in current_risk:
      print("⛲ Adaptive System: Watering Active in High-Risk Zone")
...
if __name__ == "__main__": # Simulation only runs if this file is being run directly
